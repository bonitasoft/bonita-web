  ...
  .directive('customLivingApplicationMenu', function() {
    ...
    function getTargetedApplicationPage(applicationId) {
        $http.get('../../API/living/application-page?c=1&f=token%3D'+ctrl.pageToken+'&f=applicationId%3D'+applicationId+'&d=pageId')
            .success(function(data) { 
                $scope.properties.targetUrl = $scope.properties.contextPath+"/portal/resource/app/"+ctrl.applicationToken+"/"+data[0].pageId.urlToken+"/content/"+ $window.location.search;
            });
    }
    ...
    }).then(getApplicationMenuList).then(getTargetedApplicationPage);
},
