...
package org.bonitasoft.jasperreports.engine.export;
 ...
 */
public class BonitaHtmlExporter extends JRHtmlExporter
{
	
	private static final Log log = LogFactory.getLog(BonitaHtmlExporter.class);
	...
	//changes
	public static final String PROPERTY_HTML_TAG = HTML_EXPORTER_PROPERTIES_PREFIX + "tag";
	public static final String PROPERTY_HTML_TAG_TABLE_CLASS = HTML_EXPORTER_PROPERTIES_PREFIX + "tag.table.class";
	public static final String PROPERTY_HTML_TAG_TABLE_PART = HTML_EXPORTER_PROPERTIES_PREFIX + "tag.table.part";
	public static final String PROPERTY_HTML_TAG_TABLE_PART_CLASS = HTML_EXPORTER_PROPERTIES_PREFIX + "tag.table.part.class";
	public static final String PROPERTY_HTML_TAG_TR_CLASS = HTML_EXPORTER_PROPERTIES_PREFIX + "tag.tr.class";
	public static final String PROPERTY_HTML_TAG_TR = HTML_EXPORTER_PROPERTIES_PREFIX + "tag.tr";
	public static final String PROPERTY_HTML_TAG_TD = HTML_EXPORTER_PROPERTIES_PREFIX + "tag.td";
	...
	protected class ExporterContext extends BaseExporterContext implements JRHtmlExporterContext
	{
		...
		{
			return BonitaHtmlExporter.this.getHyperlinkURL(link);
		}
	}
	 ...
	 *
	 */
	public BonitaHtmlExporter(JasperReportsContext jasperReportsContext)
	{
...
//Changes		
				String tablePartClass = null;
				String trTagClass = null;
				String tableTagClass = null;
				
//				writer.write("<tr element='2' valign=\"top\"");
// Changes
				JRPrintElement element ;
				if( gridRow != null &&  gridRow[0] != null && gridRow[0].getWrapper() != null && gridRow[0].getWrapper().getElement() != null){
				 element = gridRow[0].getWrapper().getElement();
				 if (element != null){
					 
					 tablePart = element.getPropertiesMap().getProperty(PROPERTY_HTML_TAG_TABLE_PART);
					 tablePartClass= element.getPropertiesMap().getProperty(PROPERTY_HTML_TAG_TABLE_PART_CLASS);
					 trTagClass = element.getPropertiesMap().getProperty(PROPERTY_HTML_TAG_TR_CLASS);
					 tableTagClass = element.getPropertiesMap().getProperty(PROPERTY_HTML_TAG_TABLE_CLASS); 
				...
				if(y == 0){
					...
					if(tableTagClass != null){
						writer.write(" class=\""+tableTagClass+"\"");
					}
					...
					if(tablePart != null && tablePart.length() > 0){
						writer.write("<"+tablePart );
						if(tablePartClass != null && tablePartClass.length() > 0){
							writer.write(" class=\""+tablePartClass+"\"");
						}
						writer.write(">");
					}
...
*/
	...
	protected String getCellTag(JRExporterGridCell gridCell)
	{
		...
		JRPrintElement element ;
		String tdTag = null;
		if( gridCell.getWrapper() != null && gridCell.getWrapper().getElement() != null){
		 element = gridCell.getWrapper().getElement();
		 if (element != null){
			 tdTag = element.getPropertiesMap().getProperty(BonitaHtmlExporter.PROPERTY_HTML_TAG_TD);
	 ...
	 *
	 */
	protected void exportStyledText(JRPrintText textElement, JRStyledText styledText, String tooltip, Locale locale, LineSpacingEnum lineSpacing, Float lineSpacingSize) throws IOException
	{
		...
		) throws IOException
	{
		...
		String tag = getPropertiesUtil().getProperty(textElement, PROPERTY_HTML_TAG);
...
// 		writer.write("\">");
		...
		if (textLength > 0)
		{
			...
			exportStyledText(text, styledText, textTooltip, getTextLocale(text), text.getParagraph().getLineSpacing(), text.getParagraph().getLineSpacingSize());
		}
		else
		{
			writer.write(emptyCellStringProvider.getStringForEmptyTD(imagesURI));
