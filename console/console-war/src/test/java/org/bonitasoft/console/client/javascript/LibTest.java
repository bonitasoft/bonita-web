...
public class LibTest {
    ...
    @Test
    public void testAddedResourceIsWellInjected() throws Exception {
        Mockito.doReturn("jscontent").when(resource).getText();
        Lib lib = new Lib(new JsResource(injector, resource));

        lib.inject();

        Mockito.verify(injector).inject("jscontent");
    }
}
