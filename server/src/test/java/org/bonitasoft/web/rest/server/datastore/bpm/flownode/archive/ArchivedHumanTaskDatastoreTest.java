...
public class ArchivedHumanTaskDatastoreTest {
    ...
    public void should_thrown_APIItemNotFoundException_when_ArchivedHumanTaskInstance_not_exists() throws Exception {
        ...
        ArchivedHumanTaskDatastore abstractArchivedActivityDatastore = spy(new ArchivedHumanTaskDatastore(null, ArchivedHumanTaskDefinition.TOKEN));

        doReturn(processAPI).when(abstractArchivedActivityDatastore).getProcessAPI();
        doReturn(new ArchivedAutomaticTaskInstanceImpl("test")).when(processAPI).getArchivedFlowNodeInstance(id.toLong());

        APIItemNotFoundException apiException = assertThrows(APIItemNotFoundException.class, () -> abstractArchivedActivityDatastore.get(id));
